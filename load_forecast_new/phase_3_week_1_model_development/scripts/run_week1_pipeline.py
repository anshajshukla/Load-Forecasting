...
This modified script executes the complete Week 1 baseline establishment pipeline
using relative paths for the current environment.
...
class Week1BaselineExecutor:
    ...
    def __init__(self, project_dir, data_file_path):
    ...
    def check_prerequisites(self):
        ...
        # Look for the dataset in possible locations
        if not os.path.exists(self.data_file_path):
            self.log_execution(f"⚠️ Dataset not found at primary location: {self.data_file_path}", "WARNING")
            
            # Try alternative locations
            alt_locations = [
                os.path.join(os.path.dirname(self.project_dir), "phase_2_5_feature_validation", "phase_2_5_3_outputs", "delhi_selected_features.csv"),
                os.path.join(os.path.dirname(self.project_dir), "phase_2_5_feature_validation", "phase_2_5_3_outputs", "selected_features_list.json"),
                os.path.join(os.path.dirname(os.path.dirname(self.project_dir)), "phase_2_5_feature_validation", "phase_2_5_3_outputs", "delhi_selected_features.csv")
            ]
            
            for alt_location in alt_locations:
                if os.path.exists(alt_location):
                    self.data_file_path = alt_location
                    self.log_execution(f"✅ Found dataset at alternative location: {alt_location}")
                    break
            else:
                raise FileNotFoundError(f"Required dataset not found. Please specify the correct path to delhi_selected_features.csv")
    ...
    def execute_script(self, script_info):
        ...
        try:
            # Execute the script with data file path as an environment variable
            env = os.environ.copy()
            env['DELHI_DATA_FILE'] = self.data_file_path
...
def main():
    ...
    # Get current directory and set up paths relative to it
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_dir = os.path.dirname(current_dir)  # Parent directory of scripts
    
    # Try to find the dataset in different possible locations
    data_file_paths = [
        os.path.join(os.path.dirname(os.path.dirname(project_dir)), "phase_2_5_feature_validation", "phase_2_5_3_outputs", "delhi_selected_features.csv"),
        os.path.join(os.path.dirname(project_dir), "phase_2_5_feature_validation", "phase_2_5_3_outputs", "delhi_selected_features.csv"),
        os.path.join(project_dir, "data", "delhi_selected_features.csv")
    ]
    ...
    if data_file_path is None:
        print("⚠️ WARNING: Could not automatically locate the delhi_selected_features.csv file.")
        print("Please specify the correct path when prompted.")
        data_file_path = input("Path to delhi_selected_features.csv: ")
    ...
    try:
        ...
        executor = Week1BaselineExecutor(project_dir, data_file_path)
