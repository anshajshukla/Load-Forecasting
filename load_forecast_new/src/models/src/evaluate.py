...
from sklearn.metrics import mean_squared_error, r2_score, median_absolute_error, explained_variance_score, mean_squared_log_error, mean_poisson_deviance, mean_gamma_deviance
import pickle
...
def evaluate_model(model, X_test, y_test, scalers_y, targets, save_dir='../models/saved_models', model_type=None):
    ...
    for i, target in enumerate(targets):
        ...
        # Kaggle/advanced metrics
        medae = median_absolute_error(y_test_inv[:, i], y_pred_inv[:, i])
        evs = explained_variance_score(y_test_inv[:, i], y_pred_inv[:, i])
        try:
            msle = mean_squared_log_error(np.clip(y_test_inv[:, i], a_min=0, a_max=None), np.clip(y_pred_inv[:, i], a_min=0, a_max=None))
        except ValueError:
            msle = float('nan')
        try:
            mpd = mean_poisson_deviance(np.clip(y_test_inv[:, i], a_min=0, a_max=None), np.clip(y_pred_inv[:, i], a_min=0, a_max=None))
        except ValueError:
            mpd = float('nan')
        try:
            mgd = mean_gamma_deviance(np.clip(y_test_inv[:, i], a_min=1e-9, a_max=None), np.clip(y_pred_inv[:, i], a_min=1e-9, a_max=None))
        except ValueError:
            mgd = float('nan')
        ...
        print(f"  Poisson Deviance: {mpd:.4f}")
        print(f"  Gamma Deviance: {mgd:.4f}")
        print()
