...
class LivePredictor:
    ...
    def predict_current_load(self):
        ...
        if self.scalers_y is None or len(self.scalers_y) == 0:
            print("No scalers loaded, cannot make accurate predictions")
            return None
        ...
        try:
            ...
            if X is None or len(X) == 0:
                print("Insufficient processed data for prediction")
                return None
            ...
            for i, target in enumerate(self.targets):
                if self.scalers_y and target in self.scalers_y:
                    try:
                        ...
                        print(f"Predicted {target}: {y_pred_inv[target]:.2f} MW")
                    except Exception as e:
                        print(f"Error transforming prediction for {target}: {e}")
                        y_pred_inv[target] = None
                else:
                    # If scaler not available, use the prediction as is
                    y_pred_inv[target] = y_pred[0, i]
                    print(f"Predicted {target} (no scaler): {y_pred_inv[target]:.2f}")
            
            # Get actual values if available
            latest_data = processed_data.get('latest_data')
            actual_values = {}
            
            if latest_data is not None:
                for target in self.targets:
                    if target in latest_data.columns and len(latest_data) > 0:
                        actual_values[target] = latest_data[target].iloc[-1] if not pd.isna(latest_data[target].iloc[-1]) else None
                    else:
            ...
            for target in self.targets:
                if (actual_values[target] is not None and 
                    actual_values[target] > 0 and 
                    y_pred_inv[target] is not None):
                    # MAPE - Mean Absolute Percentage Error
                    error = abs(y_pred_inv[target] - actual_values[target]) / actual_values[target] * 100
                    accuracy[target] = max(0, 100 - error)  # Convert error to accuracy, min 0%
                    print(f"Accuracy for {target}: {accuracy[target]:.2f}%")
                else:
